...
![Pie chart of BP class at six months. Which slice is bigger, Pre-hypertension or Stage 2? Now check the numbers.](../figures/dataviz/01-pie-bad.png){#fig-pie}

![The same data as a sorted horizontal bar with counts and percentages. The answer is immediate.](../figures/dataviz/01-bar-good.png){#fig-bar}
...
![Mean SBP reduction by age and BMI band, rainbow palette. The eye fights the palette every time it tries to read magnitude.](../figures/dataviz/02-rainbow-ugly.png){#fig-rainbow}

![Same data on Pequod's `log-cool` ramp. One luminance axis, one mental model.](../figures/dataviz/02-sequential-good.png){#fig-sequential}
...
![Mean SBP at six months by treatment, y-axis starting at 132 mmHg. Placebo looks twice the height of ACEi; the actual difference is about 10 mmHg.](../figures/dataviz/03-truncated-wrong.png){#fig-truncated}

![Same numbers, zero-anchored. The eye reads proportional differences honestly.](../figures/dataviz/03-honest-good.png){#fig-honest}
...
![Four donuts, one per treatment, showing the proportion controlled at six months. Which arm is highest? Which two are tied?](../figures/dataviz/04-3d-ugly.png){#fig-donut}

![A dot plot of the same percentages. ACEi tops the list. ARB and CCB are within a percentage point of each other. The visual ranking matches the numerical one.](../figures/dataviz/04-dotplot-good.png){#fig-dotplot}
...
![SBP and BMI by age band on a dual-axis chart. The two lines look correlated; they barely are.](../figures/dataviz/05-dualaxis-bad.png){#fig-dualaxis}

![Same data, two panels, free y-axes. The shapes of the two trends become legible separately.](../figures/dataviz/05-smallmult-good.png){#fig-smallmult}
...
![Mean SBP per treatment as a bar with SE whiskers. Sample size is invisible, skew is invisible, outliers are invisible.](../figures/dataviz/06-bar-mean-bad.png){#fig-meanbar}

![Same data as a boxplot with jittered points and median markers. Spread, centre, and n become visible at once.](../figures/dataviz/06-distribution-good.png){#fig-distribution}
...
![Same data faceted by treatment with the group mean overlaid. The shape of each response becomes legible.](../figures/dataviz/07-smallmult-mean-good.png){#fig-faceted}
