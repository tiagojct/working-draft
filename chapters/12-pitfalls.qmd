...
::: {.column-margin}
Each pitfall here maps to a rule in Part I. The three rules in the Results section of Chapter 2 and the graph guidance in Chapter 4 are the writing counterparts to the visual failures corrected below.
:::

With the toolkit established, each tool's failure modes need to be named. Each pair below shows the same data drawn two ways: a *bad / ugly / wrong* version, then a *good* version.
...
![Same data on Pequod's `log-cool` ramp. One luminance axis, one mental model.](../figures/dataviz/02-sequential-good.png){#fig-sequential fig-alt="The same heatmap on a single-hue sequential ramp where darker cells clearly mean larger reductions."}
...
![Mean SBP at six months by treatment, y-axis starting at 132 mmHg. Placebo looks twice the height of ACEi; the actual difference is about 10 mmHg.](../figures/dataviz/03-truncated-wrong.png){#fig-truncated fig-alt="A bar chart of mean six-month systolic by treatment with the y-axis starting at 132 mmHg, exaggerating a roughly 10 mmHg difference."}
...
sns.barplot(data=summ, x="treatment", y="mean",
            color="#FFCD00", errorbar=None, ax=ax)
...
![Four donuts, one per treatment, showing the proportion controlled at six months. Which arm is highest? Which two are tied?](../figures/dataviz/04-3d-ugly.png){#fig-donut fig-alt="Four donut charts, one per treatment, showing proportion controlled; the four are hard to rank against each other by eye."}

![A dot plot of the same percentages. ACEi tops the list. ARB and CCB are within a percentage point of each other. The visual ranking matches the numerical one.](../figures/dataviz/04-dotplot-good.png){#fig-dotplot fig-alt="A dot plot of the same four proportions on one axis: ACEi highest, ARB and CCB nearly tied, the ranking immediately clear."}
...
g = sns.relplot(data=da, x="age_band", y="value", col="metric",
                ...
                color="#1A1A1A", height=3.2, aspect=1.0)
...
crew = ["#1A1A1A", "#2B7BB9", "#D06B3A", "#3A9653"]
...
For each figure description below, identify the failure category (ugly / bad / wrong from Chapter 10) and name the specific pitfall from this chapter.
