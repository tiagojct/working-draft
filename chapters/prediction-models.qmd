...
- report a prediction model to TRIPOD+AI
- distinguish and report discrimination, calibration, and clinical usefulness
- separate development from validation performance
:::
...
Report them as such. Of the 5,204 patients, 139 with a missing discharge NT-proBNP were excluded, leaving 5,065; the model was **developed** on a random 60% of those (3,038 patients) and **validated** on the held-out 40% (2,027 patients) that played no part in fitting. Every performance number below is the validation number, because the development numbers are optimistic by construction: a model always fits the data it was trained on better than it fits anything else.
...
Discrimination is the model's ability to give higher probabilities to patients who have the event than to those who do not. The summary is the area under the ROC curve (@fig-pred-roc), equal to the c-statistic ([statistical writing](statistical-writing.qmd), on diagnostic accuracy). Here it is 0.62 on validation, down from 0.64 on development: modest, and honestly so.
...
rng = np.random.default_rng(20260515)          # stratified 60/40 split
dev_idx = []
for _, g in d.groupby("y"):
    idx = g.index.to_numpy().copy()
    rng.shuffle(idx)
    dev_idx.extend(idx[: int(0.6 * len(idx))])
dev = d.loc[d.index.isin(dev_idx)]
val = d.loc[~d.index.isin(dev_idx)]
...
3. **No clinical usefulness and no usable model.** There is no threshold analysis or decision curve, so "outperforming" is undefined in decision terms; and "available on request" ([open science](open-science.qmd)) means the model itself cannot be examined, reproduced, or applied. TRIPOD+AI asks for the full model equation or the code.
:::
