...
**Contrast.** Text drawn *on* a coloured element — a label inside a filled bar, a value on a heatmap cell, a legend key on a tinted panel — has to survive the same contrast test as body text: a ratio of at least 4.5:1 against the colour behind it (3:1 if the type is 18 pt, or 14 pt bold). Saturated mid-tone hues fail this more often than people expect. White on the Okabe-Ito vermillion `#D55E00` reaches only 3.9:1, and white on its green `#009E73` only 3.4:1, so a white label on either is unreadable for some readers even though the palette itself is colour-vision safe. The fix is to darken the fill under the label, or to switch the label to ink. A palette that passes a CVD simulation has not thereby passed a contrast check; they are different tests.
...
# Placebo in ink, the three active arms in Okabe-Ito hues.
arm_pal <- c(Placebo = "#16222a", ACEi = "#0072B2",
             ARB = "#009E73", CCB = "#D55E00")
...
from glauca import use_glauca

use_glauca("light")            # IBM Plex, pale ground, Okabe-Ito cycle

# Placebo in ink, the three active arms in Okabe-Ito hues.
pal = {"Placebo": "#16222a", "ACEi": "#0072B2",
       "ARB": "#009E73", "CCB": "#D55E00"}
