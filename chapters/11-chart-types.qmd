...
- match the six common chart types to data structures
- build each chart type in R or Python
- choose the right chart for a distribution, relationship, or effect
:::

Six chart types cover most figures in clinical and epidemiological research; each is the right choice when the data structure and the message align. The worked code uses seaborn [@waskom2021seaborn] in Python and ggplot2 [@wickham2016ggplot2] in R; VanderPlas's *Python Data Science Handbook* [@vanderplas2016pythondatascience] is a fuller reference for the Python plotting stack.
...
![Histogram with overlaid kernel density of SBP at baseline. Bin width 5 mmHg.](../figures/dataviz/type-01-histogram.png){#fig-type-histogram fig-alt="A histogram of baseline systolic blood pressure with an overlaid smooth kernel-density curve, roughly bell-shaped and centred near 148 mmHg."}
...
ggplot(cohort, aes(sbp_baseline)) +
  ...
  geom_density(colour = "#16222a", linewidth = 1.0) +
...
sns.histplot(data=cohort, x="sbp_baseline", binwidth=5,
             stat="density", color="#5b9edb", alpha=0.75,
             edgecolor="white", ax=ax)
sns.kdeplot(data=cohort, x="sbp_baseline",
            color="#16222a", linewidth=2, ax=ax)
...
palette = ["#16222a", "#0072B2", "#009E73", "#D55E00"]
...
sns.stripplot(data=cohort, x="treatment", y="dbp_baseline",
              hue="treatment", palette=palette, legend=False,
              alpha=0.30, jitter=0.16, size=3, ax=ax)
sns.boxplot(data=cohort, x="treatment", y="dbp_baseline",
            hue="treatment", palette=palette, legend=False,
            showfliers=False, fill=False, width=0.18, ax=ax)
...
ggplot(cohort, aes(sbp_baseline, dbp_baseline)) +
  geom_point(alpha = 0.30, colour = "#16222a",
             size = 1.4) +
  geom_smooth(method = "lm", se = TRUE,
              colour = "#16222a",
              fill   = "#0b62cf", alpha = 0.25) +
...
sns.regplot(data=cohort, x="sbp_baseline", y="dbp_baseline",
            scatter_kws=dict(alpha=0.30, color="#0b62cf", s=14),
            line_kws=dict(color="#16222a", linewidth=2),
            ax=ax)
...
![Line plot of mean SBP per treatment over the six-month follow-up.](../figures/dataviz/type-04-line.png){#fig-type-line fig-alt="A line plot of mean systolic blood pressure over six months for each treatment arm, the active arms declining below placebo."}
...
![Forest plot of mean Δ SBP versus Placebo by treatment arm; whiskers are 95% confidence intervals.](../figures/dataviz/type-05-forest.png){#fig-type-forest fig-alt="A forest plot of mean change in systolic blood pressure versus placebo per treatment arm, each point with horizontal 95 percent confidence-interval whiskers, active arms right of the null line."}
...
ggplot(forest_data, aes(estimate, arm)) +
  ...
  geom_errorbar(aes(xmin = lo, xmax = hi), width = 0.18,
                colour = "#16222a",
                orientation = "y") +
  geom_point(colour = "#16222a", size = 4) +
  geom_text(aes(x = hi, label = sprintf("%.1f  (%.1f – %.1f)",
                                        estimate, lo, hi)),
            hjust = 0, nudge_x = 0.4, family = "IBM Plex Mono", size = 3.3) +
...
ax.axvline(0, color="#B3BBC4", linestyle="--")
...
A Kaplan–Meier curve plots the proportion of the cohort still event-free over time, dropping in steps as events occur. Censoring (loss to follow-up) is shown by tick marks. Two or more groups can be overlaid for direct comparison of survival or event-free trajectories. @fig-type-km shows time to BP control across the four arms, computed via `survival::survfit()` with synthetic event times. Use it for time-to-event outcomes: death, hospitalisation, recurrence, or any "first time the outcome happened" measure. The classic missing element in published KM plots is the *number-at-risk* table below the x-axis; without it, the visual confidence in the right-hand tail can far exceed the actual sample size. @fig-type-km is drawn without one, so it shows only the curve geometry — which is the point here, and would not be acceptable in a paper. Add the table with `survminer::ggsurvplot(..., risk.table = TRUE)` in R, or `lifelines.plotting.add_at_risk_counts()` in Python.

![Kaplan–Meier curve of the proportion still uncontrolled over six months, by treatment arm.](../figures/dataviz/type-06-km.png){#fig-type-km fig-alt="A Kaplan–Meier step curve of the proportion still uncontrolled over six months by treatment arm, the active arms falling faster than placebo."}
...
```r
library(ggplot2); library(survival); library(dplyr)
source("glauca.R")
...
fig, ax = plt.subplots(figsize=(7.2, 4))
for arm, colour in zip(["Placebo", "ACEi", "ARB", "CCB"], palette):
