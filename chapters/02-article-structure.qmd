...
- structure a paper on the IMRAD and hourglass models
- write a structured abstract and a title that names the study
- write a limitations paragraph that discloses rather than apologises
:::
...
- **No clinical interpretation.** "We found X" belongs in the results; "this suggests Y" belongs in the discussion. The line falls between statistical and clinical reading: results may say that an estimate attenuated, that a sensitivity analysis was underpowered, or that a difference fell short of the pre-specified minimum clinically important difference, because those are properties of the analysis. What the finding means for patients, and what might explain it, waits for the discussion.

::: {.column-margin}
Choosing the right chart type and avoiding common visual errors are covered in [presenting data](04-presenting-data.qmd) and, in much greater depth, in Part II (Figures).
:::
...
Write the limitations paragraph as engineering. Most are perfunctory: a generic list of "small sample, single centre, retrospective design" that could be pasted into any paper and tells the reader nothing about *this* one [@ioannidis2007limitations]. A limitation earns its place when it is specific to your study, ranked by how much it could change the conclusion, and paired with what you did about it and what concern remains. *"Residual confounding by disease severity cannot be excluded, though we adjusted for NYHA class and LVEF and the estimate was stable in a restricted sensitivity analysis"* names the threat, the mitigation, and the honest remainder in one sentence. Separate the two kinds: threats to *internal validity* (confounding, selection and measurement bias, missing data) ask whether the finding is real in your sample; threats to *external validity* (a single centre, a narrow population, a specific era) ask whether it transports to anyone else's.
...
::: {.column-margin}
AI tools in the writing process — disclosure standards, the four-tier framework, and peer-review confidentiality — are covered in the [AI-in-writing chapter](ai-writing.qmd) later in Part I.
:::
