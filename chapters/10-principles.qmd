...
- apply the data-to-ink and self-sufficiency principles
- choose an encoding that matches the data and the message
- design a figure for the reviewer, not only for yourself
:::
...
::: {.column-margin}
The book's figures use the Glauca palette, audited for colour-vision deficiency across protanopia, deuteranopia, and tritanopia (a Viénot–Brettel–Mollon simulation [@vienot1999digital]); the categorical scale is the CVD-safe Okabe-Ito set. The design system is at [github.com/tiagojct/glauca](https://github.com/tiagojct/glauca).
:::
...
Every example in Part II uses the same simulated dataset: 600 adults with treated hypertension, allocated across four arms of unequal size (Placebo 156, ACEi 134, ARB 156, CCB 154), with blood pressure re-measured at six months and total follow-up running from 3 to 24 months. This is a different dataset from the acute heart failure cohort used for examples in Part I; the hypertension trial structure — four arms, repeated blood-pressure measurements, a binary controlled/not-controlled outcome — provides richer material for the chart-type and pitfalls chapters than the Part I cohort would have.
...
| `follow_up_months`   | num         | 3 to 24, mean 13.7                             |
| `delta_sbp`          | num         | `sbp_baseline - sbp_6m` (positive = reduction) |
| `delta_dbp`          | num         | `dbp_baseline - dbp_6m` (positive = reduction) |
| `controlled_6m`      | logical     | `sbp_6m < 140 & dbp_6m < 90`                   |
| `bp_class_6m`        | factor      | Normal / Pre-hypertension / Stage 1 / Stage 2  |

The dataset is committed at [`figures/dataviz/cohort.csv`](https://github.com/tiagojct/working-draft/blob/main/figures/dataviz/cohort.csv); the generation script is at [`figures/dv_cohort.py`](https://github.com/tiagojct/working-draft/blob/main/figures/dv_cohort.py). The Part I heart-failure cohort has a parallel generator at [`figures/sw_cohort.py`](https://github.com/tiagojct/working-draft/blob/main/figures/sw_cohort.py).

::: {.column-margin}
Part I covered when to use a figure at all and how to report the numbers that go into it. [Presenting data](04-presenting-data.qmd) is the natural companion to read before or alongside this part.
:::
