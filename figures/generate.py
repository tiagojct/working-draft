"""Generate all PNG figures for the research methods book (Glauca theme).

Figures:
  figures/sw/      — ch.4 NT-proBNP / heart failure cohort
  figures/dataviz/ — ch.11 chart types + ch.12 pitfalls

Styling is Glauca: IBM Plex Sans, a pale/white ground, the Okabe-Ito
categorical scale (CVD-safe), and the dies-blue sequential ramp. The
vendored figures/glauca.py registers the `glauca_seq` / `glauca_div`
colourmaps on import and carries the palette constants; figures/glauca.mplstyle
is its non-colour base. No pequod, no FMUP.
"""
...
import seaborn as sns  # noqa: F401  (kept: chapter code examples use it)
...
# ── register the institutional font (static IBM Plex faces bundled for Typst) ──
...
# ── Glauca palette + colourmaps (import registers glauca_seq / glauca_div) ──────
sys.path.insert(0, str(BASE))
import glauca as gl  # noqa: E402

# ══════════════════════════════════════════════════════════════════════════════
# Theme
# ══════════════════════════════════════════════════════════════════════════════

INK    = "#16222a"   # Glauca light text
MUTED  = "#55646d"   # Glauca light muted
ACCENT = "#0b62cf"   # Glauca light accent (the blue mark)
GRID   = "#dde6ea"
BAR    = "#5b9edb"    # mid dies-blue for single-series fills
PANEL  = "#ffffff"
...
# Treatment arms: ink for the placebo reference, Okabe-Ito for the active arms.
ARM_GLAUCA = {"Placebo": INK, "ACEi": "#0072B2", "ARB": "#009E73", "CCB": "#D55E00"}
...
for arm in ["Placebo", "ACEi", "ARB", "CCB"]:
    ...
    for _, r in sub.iterrows():
        ...
        for mo, val in zip(
            [0, 1, 3, 6],
            [b, b + (e - b) * 0.3 + rng.normal(0, 3),
             b + (e - b) * 0.65 + rng.normal(0, 3), e],
        ):
...
for arm in ["Placebo", "ACEi", "ARB", "CCB"]:
    ...
    e = (t <= 6).astype(int)
    t = np.minimum(t, 6.0)
    km_rows.extend({"treatment": arm, "event_time": ti, "event": ei} for ti, ei in zip(t, e))
...
def savefig(fig: plt.Figure, path: Path | str, rc: dict) -> None:
    fig.savefig(path, facecolor=rc["savefig.facecolor"], bbox_inches="tight",
                dpi=rc["savefig.dpi"])
    plt.close(fig)
...
# ── sw/01-ugly.png  (deliberately garish: default colours, all spines) ─────────
with plt.rc_context({"axes.facecolor": "#E8F4FF", "figure.facecolor": "#E8F4FF",
    ...
    bars = ax.bar(groups, means, color=["#FF0000", "#00FF00"], edgecolor="black", linewidth=1.5)
    ax.errorbar(x=range(len(groups)), y=means, yerr=sds,
...
with plt.rc_context(GLAUCA_RC):
    fig, ax = plt.subplots(figsize=(5, 4))
    bars = ax.bar(groups, means, color=BAR, edgecolor=INK, linewidth=0.7, width=0.5)
    ax.errorbar(x=range(len(groups)), y=means, yerr=sds,
                fmt="none", ecolor=INK, elinewidth=1.2, capsize=5, capthick=1.2)
    for bar, val in zip(bars, means):
        ax.text(bar.get_x() + bar.get_width() / 2, val + sds[means.index(val)] + 30,
                f"{val:,.0f}", ha="center", va="bottom", fontsize=8, color=INK)
    ax.set_ylabel("NT-proBNP at discharge (pg/mL)")
    ax.set_xlabel("")
    savefig(fig, SW_DIR / "02-bad-bar.png", GLAUCA_RC)

# ── sw/03-bad-truncated.png ───────────────────────────────────────────────────
with plt.rc_context(GLAUCA_RC):
    fig, ax = plt.subplots(figsize=(5, 4))
    bars = ax.bar(groups, medians, color=BAR, edgecolor=INK, linewidth=0.7, width=0.5)
    ...
    ax.set_ylabel("NT-proBNP at discharge (pg/mL)")
    ax.set_xlabel("")
    savefig(fig, SW_DIR / "03-bad-truncated.png", GLAUCA_RC)

# ── sw/04-good.png ────────────────────────────────────────────────────────────
with plt.rc_context(GLAUCA_RC):
    fig, ax = plt.subplots(figsize=(5.5, 4.5))
    colors_04 = ["#D55E00", "#0072B2"]  # Readmitted=vermillion, Not readmitted=blue
    positions = [1, 2]
    for pos, grp, col in zip(positions, groups, colors_04):
        ...
        ax.boxplot(vals, positions=[pos], widths=0.35, patch_artist=True, sym="",
        ...
        ax.scatter(pos, np.median(vals), marker="D", s=30, color=col, zorder=5)
    ax.set_yscale("log")
    ax.set_ylabel("NT-proBNP at discharge (pg/mL)")
    ax.set_xticks(positions)
    ax.set_xticklabels(groups)
    ax.text(0.98, 0.02, "n = 200 per group", ha="right", va="bottom",
            transform=ax.transAxes, fontsize=8, color=MUTED)
    savefig(fig, SW_DIR / "04-good.png", GLAUCA_RC)
...
# ── 01-pie-bad.png  (matplotlib defaults → deliberately weak) ──────────────────
with plt.rc_context({}):
...
with plt.rc_context(GLAUCA_RC):
    ...
    ax.barh(vc.index, vc.values, color=INK, height=0.5)
    for val, lbl in zip(vc.values, vc.index):
        ax.text(val + 3, lbl, f"{val} ({100*val/n_total:.0f}%)",
                va="center", fontsize=8.5, color=INK)
    ax.set_xlabel("Participants (n)")
    ax.set_xlim(0, vc.max() * 1.25)
    ax.grid(axis="x"); ax.grid(axis="y", visible=False)
    savefig(fig, DV_DIR / "01-bar-good.png", GLAUCA_RC)
...
hm = (
    cohort.groupby(["age_band", "bmi_band"], observed=True)["sbp_reduction"].mean()
    .reset_index().pivot(index="age_band", columns="bmi_band", values="sbp_reduction")
).reindex(index=age_labels, columns=bmi_labels)

# ── 02-rainbow-ugly.png  (rainbow stays: it is the bad example) ────────────────
with plt.rc_context(GLAUCA_RC):
    ...
    savefig(fig, DV_DIR / "02-rainbow-ugly.png", GLAUCA_RC)

# ── 02-sequential-good.png  (Glauca dies-blue sequential ramp) ─────────────────
with plt.rc_context(GLAUCA_RC):
    fig, ax = plt.subplots(figsize=(6.5, 4))
    im = ax.imshow(hm.values, cmap="glauca_seq", aspect="auto")
    ax.set_xticks(range(len(bmi_labels))); ax.set_xticklabels(bmi_labels)
    ax.set_yticks(range(len(age_labels))); ax.set_yticklabels(age_labels)
    ax.set_xlabel("BMI band (kg/m²)"); ax.set_ylabel("Age band")
    thresh = np.nanmean(hm.values)
    for i in range(len(age_labels)):
        for j in range(len(bmi_labels)):
            val = hm.values[i, j]
            if not np.isnan(val):
                ax.text(j, i, f"{val:.1f}", ha="center", va="center",
                        fontsize=8, color="white" if val > thresh else INK)
    cbar = plt.colorbar(im, ax=ax, shrink=0.8)
    cbar.set_label("Δ SBP (mmHg)", fontsize=9)
    ax.grid(visible=False)
    savefig(fig, DV_DIR / "02-sequential-good.png", GLAUCA_RC)

# ── mean SBP per arm helper ────────────────────────────────────────────────────
arm_order = ["Placebo", "ACEi", "ARB", "CCB"]
arm_sbp = cohort.groupby("treatment")["sbp_6m"].mean().reindex(arm_order)

# ── 03-truncated-wrong.png ────────────────────────────────────────────────────
with plt.rc_context(GLAUCA_RC):
    fig, ax = plt.subplots(figsize=(5, 4))
    bars = ax.bar(arm_order, arm_sbp.values, color=BAR, edgecolor=INK, linewidth=0.7, width=0.5)
    ...
    savefig(fig, DV_DIR / "03-truncated-wrong.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for bar, val in zip(bars, arm_sbp.values):
        ax.text(bar.get_x() + bar.get_width()/2, val + 1.5, f"{val:.1f}",
                ha="center", va="bottom", fontsize=8.5, color=INK)
    ax.set_xlabel("Treatment"); ax.set_ylabel("SBP at 6 months (mmHg)")
    savefig(fig, DV_DIR / "03-honest-good.png", GLAUCA_RC)

# ── controlled pct per arm helper ─────────────────────────────────────────────
ctrl_pct = (
    cohort.groupby("treatment")["controlled_6m"]
    .apply(lambda s: 100 * s.astype(bool).mean()).reindex(arm_order)
)

# ── 04-3d-ugly.png  (matplotlib defaults, gratuitous donuts) ───────────────────
tab10 = matplotlib.colormaps["tab10"]
ugly_colors = {"Placebo": tab10(0), "ACEi": tab10(1), "ARB": tab10(2), "CCB": tab10(3)}
with plt.rc_context({}):
    from matplotlib.patches import PathPatch
    from matplotlib.path import Path as MPath
    fig, axes = plt.subplots(2, 2, figsize=(7, 6))
    for ax, arm in zip(axes.flat, arm_order):
        pct = ctrl_pct[arm]; rest = 100 - pct; col = ugly_colors[arm]
        for start, size, fc in [(0, pct / 100, col), (pct / 100, rest / 100, "lightgray")]:
            angles = np.linspace(start * 2 * np.pi, (start + size) * 2 * np.pi, 100)
            xs_o = np.cos(angles); ys_o = np.sin(angles)
            xs_i = np.cos(angles[::-1]) * 0.55; ys_i = np.sin(angles[::-1]) * 0.55
            ...
            ax.add_patch(PathPatch(MPath(verts, codes), facecolor=fc, edgecolor="white", linewidth=0.5))
...
with plt.rc_context(GLAUCA_RC):
    ...
    for i, (arm, val) in enumerate(zip(sorted_arms, sorted_vals)):
        ...
        ax.scatter(val, i, color=INK, s=55, zorder=3)
        ax.text(val + 0.8, i, f"{val:.0f}%", va="center", fontsize=8.5, color=INK)
    ax.set_yticks(range(len(sorted_arms))); ax.set_yticklabels(sorted_arms)
    ax.set_xlim(0, max(sorted_vals) * 1.22)
    ax.set_xlabel("Controlled at 6 months (%)")
    ax.grid(axis="x"); ax.grid(axis="y", visible=False)
    savefig(fig, DV_DIR / "04-dotplot-good.png", GLAUCA_RC)

# ── age_band SBP / BMI helpers ────────────────────────────────────────────────
age_band_stats = cohort.groupby("age_band", observed=True).agg(
    sbp_mean=("sbp_6m", "mean"), bmi_mean=("bmi", "mean")).reindex(age_labels)

# ── 05-dualaxis-bad.png ───────────────────────────────────────────────────────
with plt.rc_context(GLAUCA_RC):
    fig, ax1 = plt.subplots(figsize=(6, 4))
    ax2 = ax1.twinx()
    ax1.plot(age_labels, age_band_stats["sbp_mean"], color=INK, marker="o", linewidth=2, label="SBP (mmHg)")
    ax2.plot(age_labels, age_band_stats["bmi_mean"], color="#0072B2", marker="s",
             linewidth=2, label="BMI (kg/m²)", linestyle="--")
    ax1.set_xlabel("Age band")
    ax1.set_ylabel("SBP at 6 months (mmHg)", color=INK)
    ax2.set_ylabel("Mean BMI (kg/m²)", color="#0072B2")
    ax2.tick_params(axis="y", labelcolor="#0072B2")
    ax2.spines["right"].set_visible(True); ax2.spines["right"].set_color("#0072B2")
    l1, b1 = ax1.get_legend_handles_labels(); l2, b2 = ax2.get_legend_handles_labels()
    ax1.legend(l1 + l2, b1 + b2, loc="upper left", fontsize=8)
    savefig(fig, DV_DIR / "05-dualaxis-bad.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for ax, col, title, ylabel in [
        ...
        ax.plot(age_labels, age_band_stats[col], color=INK, marker="o", linewidth=2)
        ax.set_xlabel("Age band"); ax.set_title(title, fontsize=10)
        ax.set_ylabel(ylabel); ax.tick_params(axis="x", rotation=15)
    plt.tight_layout()
    savefig(fig, DV_DIR / "05-smallmult-good.png", GLAUCA_RC)

# ── mean + SE SBP per arm helper ──────────────────────────────────────────────
arm_se = cohort.groupby("treatment")["sbp_6m"].agg(["mean", "sem"]).reindex(arm_order)

# ── 06-bar-mean-bad.png ───────────────────────────────────────────────────────
with plt.rc_context(GLAUCA_RC):
    fig, ax = plt.subplots(figsize=(5, 4))
    ax.bar(arm_order, arm_se["mean"].values, color=BAR, edgecolor=INK, linewidth=0.7, width=0.5)
    ax.errorbar(x=range(len(arm_order)), y=arm_se["mean"].values, yerr=arm_se["sem"].values,
                fmt="none", ecolor=INK, elinewidth=1.2, capsize=5, capthick=1.2)
    ax.set_xlabel("Treatment"); ax.set_ylabel("SBP at 6 months (mmHg)")
    savefig(fig, DV_DIR / "06-bar-mean-bad.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for arm in arm_order:
        ...
        col = ARM_GLAUCA[arm]; pos = arm_order.index(arm)
        jit = rng.uniform(-0.18, 0.18, len(vals))
        ax.scatter(pos + 1 + jit, vals, alpha=0.20, s=8, color=col, zorder=2)
        ax.boxplot(vals, positions=[pos + 1], widths=0.35, patch_artist=True, sym="",
                   medianprops={"linewidth": 0},
    ...
    ax.set_xticks(range(1, len(arm_order) + 1)); ax.set_xticklabels(arm_order)
    ax.set_ylabel("SBP at 6 months (mmHg)")
    savefig(fig, DV_DIR / "06-distribution-good.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for arm in arm_order:
        ...
        for pid in sub["id"].unique():
            ...
            ax.plot(row["month"], row["sbp"], color=ARM_GLAUCA[arm], alpha=0.35, linewidth=0.7)
    handles = [mpatches.Patch(color=ARM_GLAUCA[a], label=a) for a in arm_order]
    ax.legend(handles=handles, loc="upper right")
    ax.set_xlabel("Months from baseline"); ax.set_ylabel("SBP (mmHg)")
    ax.set_xticks([0, 1, 3, 6])
    savefig(fig, DV_DIR / "07-spaghetti-ugly.png", GLAUCA_RC)

# ── 07-smallmult-mean-good.png ────────────────────────────────────────────────
with plt.rc_context(GLAUCA_RC):
    fig, axes = plt.subplots(1, 4, figsize=(11, 3.5), sharey=True)
    for ax, arm in zip(axes, arm_order):
        sub = traj[traj["treatment"] == arm]; col = ARM_GLAUCA[arm]
        for pid in sub["id"].unique():
            row = sub[sub["id"] == pid].sort_values("month")
        ...
        ax.set_title(arm, fontsize=10); ax.set_xticks([0, 1, 3, 6]); ax.set_xlabel("Months")
        if ax is axes[0]:
            ax.set_ylabel("SBP (mmHg)")
    plt.tight_layout()
    savefig(fig, DV_DIR / "07-smallmult-mean-good.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    ax.hist(sbp_vals, bins=bins, density=True, color=BAR, alpha=0.75, edgecolor="white", linewidth=0.5)
    kde_x = np.linspace(sbp_vals.min(), sbp_vals.max(), 300)
    ax.plot(kde_x, stats.gaussian_kde(sbp_vals)(kde_x), color=INK, linewidth=2)
    ax.set_xlabel("SBP at baseline (mmHg)"); ax.set_ylabel("Density")
    savefig(fig, DV_DIR / "type-01-histogram.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for arm in arm_order:
        ...
        col = ARM_GLAUCA[arm]; pos = arm_order.index(arm)
        vp = ax.violinplot(vals, positions=[pos + 1], widths=0.6, showextrema=False, showmedians=False)
        for body in vp["bodies"]:
            body.set_facecolor(col); body.set_alpha(0.18); body.set_edgecolor(col); body.set_linewidth(0.8)
        jit = rng.uniform(-0.14, 0.14, len(vals))
        ax.scatter(pos + 1 + jit, vals, alpha=0.25, s=8, color=col, zorder=2)
        ax.boxplot(vals, positions=[pos + 1], widths=0.28, patch_artist=True, sym="",
                   medianprops={"linewidth": 0},
                   whiskerprops={"color": col, "linewidth": 0.8},
                   capprops={"color": col, "linewidth": 0.8},
                   boxprops={"facecolor": "none", "edgecolor": col, "linewidth": 0.8})
    ...
    savefig(fig, DV_DIR / "type-02-distribution.png", GLAUCA_RC)

# ── type-03-scatter.png ───────────────────────────────────────────────────────
with plt.rc_context(GLAUCA_RC):
    fig, ax = plt.subplots(figsize=(5.5, 4.5))
    x = cohort["sbp_baseline"].values; y = cohort["dbp_baseline"].values
    ax.scatter(x, y, color=INK, alpha=0.30, s=12, zorder=2)
    slope, intercept, *_ = stats.linregress(x, y)
    x_line = np.linspace(x.min(), x.max(), 200); y_line = slope * x_line + intercept
    ax.plot(x_line, y_line, color=ACCENT, linewidth=2, zorder=3)
    n = len(x); x_bar = x.mean()
    se_y = np.sqrt(np.sum((y - (slope * x + intercept))**2) / (n - 2)
                   * (1/n + (x_line - x_bar)**2 / np.sum((x - x_bar)**2)))
    ax.fill_between(x_line, y_line - 1.96 * se_y, y_line + 1.96 * se_y, color=ACCENT, alpha=0.15, zorder=1)
    ax.set_xlabel("SBP at baseline (mmHg)"); ax.set_ylabel("DBP at baseline (mmHg)")
    savefig(fig, DV_DIR / "type-03-scatter.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for arm in arm_order:
        ...
        ax.plot(sub["month"], sub["sbp"], color=ARM_GLAUCA[arm], marker="o", linewidth=2, label=arm)
    ax.set_xticks([0, 1, 3, 6]); ax.set_xlabel("Months from baseline"); ax.set_ylabel("SBP (mmHg)")
    ax.legend(loc="upper right")
    savefig(fig, DV_DIR / "type-04-line.png", GLAUCA_RC)

# ── type-05-forest.png ────────────────────────────────────────────────────────
import statsmodels.formula.api as smf
cohort["treatment_cat"] = pd.Categorical(cohort["treatment"], categories=arm_order)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for yp, arm, est, lo, hi in zip(y_pos, forest_arms, estimates, ci_lo, ci_hi):
        ax.hlines(yp, lo, hi, color=INK, linewidth=1.5)
        ax.scatter(est, yp, color=INK, s=80, zorder=5)
        ax.text(hi + 0.3, yp, f"{est:.1f} ({lo:.1f} – {hi:.1f})", va="center",
                fontsize=8, fontfamily="monospace", color=INK)
    ax.axvline(0, color=MUTED, linewidth=1, linestyle="--", zorder=1)
    ax.set_yticks(list(y_pos)); ax.set_yticklabels(forest_arms)
    ax.set_xlabel("Δ SBP (mmHg, vs Placebo)")
    ax.grid(axis="y", visible=False); ax.grid(axis="x")
    savefig(fig, DV_DIR / "type-05-forest.png", GLAUCA_RC)
...
with plt.rc_context(GLAUCA_RC):
    ...
    for arm in arm_order:
        ...
        kmf = KaplanMeierFitter(); kmf.fit(sub["event_time"], sub["event"], label=arm)
        sf = kmf.survival_function_[arm].values * 100
        ax.step(kmf.timeline, sf, where="post", color=ARM_GLAUCA[arm], linewidth=2, label=arm)
    ax.set_xlim(0, 6); ax.set_ylim(0, 105)
    ax.set_xlabel("Months from baseline"); ax.set_ylabel("Uncontrolled (%)")
    ax.set_xticks([0, 1, 2, 3, 4, 5, 6]); ax.legend(loc="upper right")
    savefig(fig, DV_DIR / "type-06-km.png", GLAUCA_RC)


# ══════════════════════════════════════════════════════════════════════════════
# Theme comparison  (ch.14): matplotlib default vs Glauca — same data, same shape
# ══════════════════════════════════════════════════════════════════════════════
print("\n── theme-comparison ──────────────────────────────────────────────────────")

def _draw_distrib_panel(ax, colors_map, rng_ref):
    for arm in arm_order:
        vals = cohort[cohort["treatment"] == arm]["dbp_baseline"].values
        col = colors_map[arm]; pos = arm_order.index(arm)
        vp = ax.violinplot(vals, positions=[pos + 1], widths=0.6, showextrema=False, showmedians=False)
        for body in vp["bodies"]:
            body.set_facecolor(col); body.set_alpha(0.18); body.set_edgecolor(col); body.set_linewidth(0.8)
...
# Left: matplotlib defaults (garish, boxed). Right: Glauca.
with plt.rc_context({}):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4.5))
    fig.patch.set_facecolor("white")
    default_map = {"Placebo": tab10(0), "ACEi": tab10(1), "ARB": tab10(2), "CCB": tab10(3)}
    _draw_distrib_panel(ax1, default_map, np.random.default_rng(42))
    ax1.set_title("matplotlib default", fontsize=10, pad=8)

    ax2.set_facecolor(PANEL)
    ax2.spines["top"].set_visible(False); ax2.spines["right"].set_visible(False)
    ax2.spines["bottom"].set_edgecolor(MUTED); ax2.spines["left"].set_edgecolor(MUTED)
    ax2.tick_params(colors=MUTED)
    ax2.yaxis.label.set_color(INK); ax2.xaxis.label.set_color(INK)
    ax2.grid(True, axis="y", color=GRID, linewidth=0.6)
    _draw_distrib_panel(ax2, ARM_GLAUCA, np.random.default_rng(42))
    ax2.set_title("Glauca theme", fontsize=10, color=INK, pad=8)
    for lbl in ax2.get_xticklabels() + ax2.get_yticklabels():
        lbl.set_color(MUTED)
    ax2.yaxis.label.set_color(INK)
    plt.tight_layout(pad=1.5)
    fig.savefig(DV_DIR / "theme-comparison.png", facecolor="white", bbox_inches="tight", dpi=150)
    plt.close(fig)
