...
quarto render                        # full book build → _book/
...
[.github/workflows/deploy.yml](.github/workflows/deploy.yml) builds and deploys to GitHub Pages on push to `main`. [.github/workflows/check.yml](.github/workflows/check.yml) does a render-only check on every pull request and uploads `_book/` as an artifact — broken citations or missing figure paths fail the check before merge.
...
- **Two bibliographies.** [assets/refs-sw.bib](assets/refs-sw.bib) backs Part I (writing), [assets/refs-dataviz.bib](assets/refs-dataviz.bib) backs Part II (figures). Citations use Nature style via [assets/nature.csl](assets/nature.csl). When adding a citation, put the entry in the bib file that matches the part the chapter belongs to.
...
- **Theme.** [assets/custom.scss](assets/custom.scss) layers an FMUP (Faculty of Medicine, University of Porto) institutional palette on top of the `cosmo` Bootstrap theme — yellow header stripe (`#FFCD00`), Atkinson Hyperlegible for body, Geist Mono for code. [assets/head.html](assets/head.html) adds Matomo analytics and Google Fonts preconnects and is injected via `include-in-header`.
...
- **Simulated cohorts are part of the book's didactic conceit.** Part I uses a fictional heart failure / NT-proBNP cohort generated by [figures/sw_cohort.py](figures/sw_cohort.py) → `figures/sw/cohort.csv`; Part II uses a fictional hypertension cohort generated by [figures/dv_cohort.py](figures/dv_cohort.py) → `figures/dataviz/cohort.csv`. Both scripts are deterministic (`SEED = 20260515`). They are intentionally simulated for the worked examples — do not treat them as real data or suggest replacing them with a real dataset.
